package kr.ac.sunmoon.hunminjeongeum_server.hint.hint
...
class HintService(
    ...
    // *추가됨*
    // 서버용 진입 함수, 원본 문자, 초성 퀴즈, 카테고리를 매개변수로 받고 힌트를 생성함
    ...
    ) : RoundHints
    {
        ...
        val validation = HintValidator.validateWord(entry)

        if (!validation.valid) {
            throw HintServiceException(validation.reason)
        }

        val chosungHints = RevealPlanner.plan(
            word = word,
            wordQuiz = wordQuiz,
            random = random
        )
    ...
    private suspend fun schedule(category: String): Deferred<RoundHints> {
        ...
        // 테스트 재현성이 깨질 수 있습니다.
        val chosungHints = RevealPlanner.plan(entry.word, entry.wordQuiz, random)
