...
object RevealPlanner {
    ...
    // *수정됨*
    //
    fun plan(word: String,
             wordQuiz : String,
             random: Random): List<String> {

        val indices = planIndices(word, random)

        return buildHints(word, wordQuiz, indices)
    }

    // 서버에서 전달받은 wordQuiz(초성문제)를 바탕으로 일부 문제만 공개
    fun buildHints(word: String,
                   wordQuiz : String,
                   indices: List<Int>): List<String>
    {
        require(word.length == wordQuiz.length)
